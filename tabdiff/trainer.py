...
from synthius.TabDiff.utils_train import update_ema
...
class Trainer:
    def __init__(
            ...
            evaluate_synthetic_data: bool = False,
            **kwargs
    ):
        self.y_only = y_only
        ...
        self.curr_epoch = int(
            os.path.basename(self.ckpt_path).split('_')[-1].split('.')[0]) if self.ckpt_path is not None else 0
        self.evaluate_synthetic_data = evaluate_synthetic_data
        ...
        for epoch in range(start_epoch, self.steps):
            ...
            if self.dataset.d_numerical > 0:  # numerical data is not empty
                ...
                if self.diffusion.num_schedule.rho().dim() == 0:  # non-learnable num schedule
                    ...
                    num_noise_dict = {f"num_noise/rho_col_{i}": value.item() for i, value in
                                      enumerate(self.diffusion.num_schedule.rho())}
            ...
            if len(self.dataset.categories) > 0:  # categorical data is not empty
                ...
                if self.diffusion.cat_schedule.k().dim() == 0:  # non-learnable cat schedule
                    ...
                    cat_noise_dict = {f"cat_noise/k_col_{i}": value.item() for i, value in
                                      enumerate(self.diffusion.cat_schedule.k())}
            ...
            update_ema(self.ema_num_schedule.parameters(), self.diffusion.num_schedule.parameters(),
                       rate=self.ema_decay)
            update_ema(self.ema_cat_schedule.parameters(), self.diffusion.cat_schedule.parameters(),
                       rate=self.ema_decay)
            ...
            if total_loss < best_loss and self.curr_epoch > self.steps / 2:
                ...
                torch.save(state_dicts,
                           os.path.join(self.model_save_path, f'best_model_{np.round(total_loss, 4)}_{epoch + 1}.pt'))
                patience = 0
            else:
            ...
            print("Current epoch", self.curr_epoch)
            print("Ema total loss", ema_total_loss)
            # Save the best ema ckpt
            if ema_total_loss < best_ema_loss and self.curr_epoch > self.steps / 2:  # previously was 4000?
                ...
                torch.save(state_dicts, os.path.join(self.model_save_path,
            ...
            if (epoch + 1) % self.check_val_every == 0:
                ...
                if self.evaluate_synthetic_data:
                    print_with_bar(
                        f"Routine Generation Evaluation every {self.check_val_every}, "
                        f"currently at epoch #{epoch + 1}, wiht total_loss={total_loss}.")
    ...
    def evaluate_generation(self, save_metric_details=False, plot_density=False, ema=False):
        ...
        syn_df_loaded = pd.read_csv(os.path.join(save_path,
                                                 "samples.csv"))  # In the original tabsyn code, syn_data is implicitly casted into float.64 when it gets loaded with pd.read_csv in the evaluation script. If we don't cast, the density evluation for some columns (especially those with tailed and peaked distribution) will collapse.
        ...
        with open(path, "w") as json_file:
            json.dump(out_metrics, json_file, indent=4,
                      separators=(", ", ": "))  # always locally save the output metrics
    ...
    def sample(self, num_samples: int, keep_nan_samples: bool = False):
        return self.diffusion.sample_all(num_samples, self.sample_batch_size, keep_nan_samples=keep_nan_samples)
    ...
    def sample_synthetic(self, num_samples, keep_nan_samples=True, ema=False):
        ...
        syn_data = self.sample(num_samples=num_samples, keep_nan_samples=keep_nan_samples)
        ...
        num_nan_cols = self.dataset.num_nan_cols
        cat_nan_cols = self.dataset.cat_nan_cols
        ...
        if self.y_only:
            ...
            def fill_w_nans(nan_cols, data):
                for i, prop in nan_cols.items():
                    filled_value = prop["filled_value"]
                    missing_proportion = prop["missing_proportion"]
                    if missing_proportion > 0:
                        missing_mask = (data[:, i] == filled_value) & (np.random.random(len(data)) < missing_proportion)
                        if missing_mask.any():
                            data[missing_mask, i] = np.nan
                return data

            # Reintroduce nans
            if num_nan_cols is not None:
                syn_num = fill_w_nans(num_nan_cols, syn_num)
                print("Reintroduced NaNs in categorical columns.")

            if cat_nan_cols is not None:
                syn_cat = fill_w_nans(cat_nan_cols, syn_cat)
                print("Reintroduced NaNs in numerical columns.")
            ...
            if self.id_col is not None:
                ...
                random_ids = [f"ID-{rng.integers(10000000, 99999999)}" for _ in range(len(syn_df))]
                syn_df.loc[:, self.id_col] = random_ids
                print("Generated IDs")
    ...
    def test_impute(self, trail_start, trial_size, resample_rounds, impute_condition, imputed_sample_save_dir, w_num,
                    w_cat):
        self.diffusion.eval()
        ...
        with torch.no_grad():
            ...
            for trial in range(trail_start, trail_start + trial_size):
                ...
                if cat_mask_idx:
                    x_cat_test[:, cat_mask_idx] = \
                    torch.tensor(categories, dtype=x_cat_test.dtype, device=x_cat_test.device)[cat_mask_idx]

                # Sample imputed tables
                syn_data = self.diffusion.sample_impute(x_num_test, x_cat_test, num_mask_idx, cat_mask_idx,
                                                        resample_rounds, impute_condition, w_num, w_cat)
                ...
                if torch.any((syn_data[:, d_numerical + 1:]).max(dim=0).values > (x_cat_train[:, 1:]).max(
                        dim=0).values):  # if the test set contains categories not presented in the train set, we can not do cat_inverse. So we implement a patch that set those columns to the same as the train set
                    print(
                ...
                syn_num, syn_cat, syn_target = split_num_cat_target(syn_data, info, num_inverse, int_inverse,
                                                                    cat_inverse)
                syn_df = recover_data(syn_num, syn_cat, syn_target, info)
