...
def preprocess_adult():
    with open(f'{INFO_PATH}/adult.json', 'r') as f:
        info = json.load(f)
    print("Do nothing...")
    pass
...
def preprocess_diabetes():
    ...
    train_df = pd.DataFrame(np.concatenate([X_cont_train, X_cat_train, y_train], axis=1),
                            columns=num_features + cat_features + [target])
    val_df = pd.DataFrame(np.concatenate([X_cont_val, X_cat_val, y_val], axis=1),
                          columns=num_features + cat_features + [target])
    test_df = pd.DataFrame(np.concatenate([X_cont_test, X_cat_test, y_test], axis=1),
                           columns=num_features + cat_features + [target])
...
def process_data(name):
    ...
    val_df = pd.DataFrame(columns=data_df.columns).astype(
        data_df.dtypes)  # by default (val_path is not provided), set val_Df to be empty
    if info['test_path']:

        # if testing data is given
        test_path = info['test_path']

        if "adult_" in name:  # BUG: currently data saved at adult's test_path cannot be directly loaded. Consider integrate the following code to a preprocesing function for adult
    ...
    if info["id_col"]:
        try:
            train_df = train_df.drop([info["id_col"]], axis=1)
            test_df = test_df.drop([info["id_col"]], axis=1)
